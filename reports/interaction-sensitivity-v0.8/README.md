...
차이는 **탐색 예산**이다. v0.3/v0.5env는 256으로 돌았고 이 격자는 1024다. 예산 스케일링
연구에서 256 → 1024가 격차를 약 +0.011 올린다는 것을 이미 쟀는데, 그 이동이 기준선을
0 위로 밀어 올린다. 즉 **"가정을 흔들면 부호가 바뀐다"는 v0.3의 서술 중 일부는 탐색
해상도 부족의 결과**였다.
